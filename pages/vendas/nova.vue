<template>
          ...
          <div v-else class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(item, index) in sale.items" :key="index" class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button @click="editItem(index)" class="text-blue-600 hover:text-blue-900 mr-3">
                      Editar
                    </button>
            ...
            <div>
              ...
              <div class="flex justify-between py-2 border-b border-gray-200">
                ...
                <div class="flex items-center">
                  ...
                  />
                  <span class="text-sm font-medium text-gray-900">{{ formatCurrency(sale.discount) }}</span>
                </div>
              </div>
              <div class="flex justify-between py-2 font-medium">
                <span class="text-gray-900">Total:</span>
                <span class="text-lg text-blue-600">{{ formatCurrency(total) }}</span>
              </div>
            </div>
        ...
        <div class="grid grid-cols-1 gap-4">
          <div>
            ...
            >
              ...
              >
                {{ product.name }} {{ !isProductAvailable(product) ? '(Sem estoque)' : '' }}
              </option>
            </select>
  ...
  import { saleHelpers, SaleStatus, PaymentMethod, CreateSaleDTO } from '~/types/sale';
  import { CustomerType, CreateCustomerDTO } from '~/types/customer';
  ...
  const fetchCustomers = async () => {
    try {
      ...
      customers.value = [
        { id: '1', name: 'João Silva', email: 'joao@email.com', type: CustomerType.PHYSICAL },
        { id: '2', name: 'Maria Oliveira', email: 'maria@email.com', type: CustomerType.PHYSICAL },
        { id: '3', name: 'Empresa XYZ Ltda', email: 'contato@xyz.com', type: CustomerType.LEGAL },
        { id: '4', name: 'Ana Ferreira', email: 'ana@email.com', type: CustomerType.PHYSICAL },
        { id: '5', name: 'Tech Solutions S.A.', email: 'contato@techsolutions.com', type: CustomerType.LEGAL },
      ];
    } catch (error) {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Mock de dados para desenvolvimento
      products.value = [
        { id: '1', name: 'Notebook Dell XPS', sku: 'DELL001', price: 7500, stock: 5, type: ProductType.PRODUCT },
        { id: '2', name: 'Mouse Wireless', sku: 'MOUSE001', price: 120, stock: 20, type: ProductType.PRODUCT },
        { id: '3', name: 'Monitor Ultrawide', sku: 'MON001', price: 2300, stock: 8, type: ProductType.PRODUCT },
        { id: '4', name: 'Teclado Mecânico', sku: 'TEC001', price: 350, stock: 15, type: ProductType.PRODUCT },
        { id: '5', name: 'Headset Gamer', sku: 'HEAD001', price: 420, stock: 10, type: ProductType.PRODUCT },
        { id: '6', name: 'Configuração de Equipamento', sku: 'SERV001', price: 150, type: ProductType.SERVICE },
        { id: '7', name: 'Manutenção Preventiva', sku: 'SERV002', price: 250, type: ProductType.SERVICE },
      ];
    } catch (error) {
  ...
  const isProductAvailable = (product) => {
    ...
    if (editingItemIndex.value !== null) {
      ...
      const otherItems = sale.value.items.filter((_, index) => index !== editingItemIndex.value);
  ...
  const updateItemTotal = () => {
    ...
    currentItem.value.total = saleHelpers.calculateItemTotal(quantity, price, discount);
  };
  ...
  const addOrUpdateItem = () => {
    ...
    // Mostrar notificação
    notification.success(`Item ${editingItemIndex.value !== null ? 'atualizado' : 'adicionado'} com sucesso!`);
  };
  ...
  const validateCustomer = (): boolean => {
    ...
    if (newCustomer.value.document) {
      const isValid = newCustomer.value.type === CustomerType.PHYSICAL
        ? customerHelpers.isValidCPF(newCustomer.value.document)
        : customerHelpers.isValidCNPJ(newCustomer.value.document);
      
      if (!isValid) {
        customerErrors.value.document = newCustomer.value.type === CustomerType.PHYSICAL
          ? 'CPF inválido'
          : 'CNPJ inválido';
      }
    }
